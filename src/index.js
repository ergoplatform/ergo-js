...
function output_bytes(out, distinct_tokens){
  var res = int_to_vlq(out['value'])
  ...
  var k = out['additionalRegisters'].length
...
function input_bytes(i){
  var res = Buffer.from(i['boxId'], 'hex')
  var sp = i['spendingProof']
...
function serialize_tx(tx) {
  var res = int_to_vlq(tx['inputs'].length)
  ...
  for(var i in tx['dataInputs']){
    res = Buffer.concat([res, Buffer.from(i['boxId'], 'hex')])
  }

  var distinct_ids = distinct_token_list(tx['outputs'])

  res = Buffer.concat([res, int_to_vlq(distinct_ids.length)])
  for(var d in distinct_ids){
...
export function formTransaction(recipient, amount, fee, boxesToSpend, chargeAddress, height) {
  ...
  let signedTransaction = Object.assign({}, unsignedTransaction)

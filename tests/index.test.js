...
const complexBoxesAddress = '9fRusAarL1KkrWQVsxSRVYnvWxaAT2A96cKtNn9tvPh5XUyCisr';
const complexBoxes = [
  {
...
test('simple address validity test', () => {
  ...
  addresses.forEach((address) => {
    expect(checkAddressValidity(address[0])).toBe(address[1]);
  });
});
...
describe('getBoxesFromAddress test', () => {
  ...
  it('should return boxes with assets and registers', async () => {
    const mock = new MockAdapter(testNetServer);
    mock.onGet(`/transactions/boxes/byAddress/unspent/${complexBoxesAddress}`)
      .reply(200, complexBoxes);
    const res = await getBoxesFromAddress(complexBoxesAddress, true);
